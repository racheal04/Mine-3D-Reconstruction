...
| 根因 | 说明 | 严重程度 |
|------|------|----------|
| **几何退化** | 矿井长直隧道是近似圆柱体，激光在所有高度看到的墙壁都一样。LiDAR 在 Z 方向没有几何约束。 | ★★★★★ |
| **ESKF 退化方向信任 IMU** | 卡尔曼滤波在激光无法约束的方向自动信任 IMU 预测。IMU 加速度计偏置随时间二次积分，80m 走下来 Z 漂了 1.5m。 | ★★★★ |
| **IMU 噪声参数错误** | 默认 `acc_cov=0.1`，但 LITEF LCI-100N 光纤陀螺真实噪声仅 `6.6e-3` m/s²，差 15 倍。滤波器过度不确定 IMU 输入。 | ★★★ |
| **外参标定缺失** | LiDAR→IMU 的平移向量 `[-0.268, -0.380, 0.031]` 之前未设置（默认 identity），转动时力臂效应引入误差。 | ★★ |

### 三层解决

```
第一层: 正确标定参数 (直接减轻)
  ├── extrinsic_T: 来自 003_With_software_tools.txt
  └── IMU 噪声: 来自 003_LITEF_imu_intrinsic.yaml (光纤陀螺真实参数)
         ↓
第二层: GTSAM 位姿图回环 (事后修正)
  ├── ScanContext: 检测回环 (SC 距离 < 0.22)
  ├── PCL ICP: 验证回环 (fitness < 0.4)
  ├── GTSAM ISAM2: 因子图增量优化
  └── BetweenFactor: T_rel = identity + ICP rotation
         ↓
第三层: 两遍处理 (工程化保证收敛)
  ├── 第一遍: 建立回环约束图
  └── 第二遍: 从修正状态递推, 轨迹闭合
...
1. **ISAM2 增量优化在单遍处理中难以收敛**：早期关键帧在回环检测之前已被线性化，初始点离真值太远，陷入局部最优。

2. **两遍处理的原理**：第一遍 ISAM2 内部积累了回环约束和修正方向。第二遍新的关键帧从修正后的位姿递推（`prev_corrected * rel_odom`），初始猜测就接近真值，自动闭合。

3. **光纤陀螺的精度优势**：LITEF 噪声 5000 倍低于默认参数。滤波器和 IMU 实际特性匹配后，短期 Z 漂移大幅减小，回环只需要修正剩余的缓慢累积误差。

### 回环检测实现

| 组件 | 状态 | 说明 |
|------|------|------|
| ScanContext 回环检测 | ✅ 已验证 | SC 距离 <0.25, ICP fitness <0.001 |
| GTSAM ISAM2 位姿图 | ✅ 已验证 | 两遍处理后 dz 从 1.5m → 0m |
| 实施方案 | 无需重启节点, 跑两遍 bag | 第一遍建图, 第二遍自动闭合 |
...
| 机器狗 | 移动平台 | ✅ 已有 |
| RTK GPS | Z 轴绝对约束 | ✅ 已有 |
| LiDAR (型号待确认) | 3D 点云 | 需确认线数/频率 |
| IMU (型号待确认) | 惯性导航 | 需确认是否与 LiDAR 刚性连接 |
| 笔记本 | 计算 + 存储 | ✅ 已有 |
...
| 要求 | 原因 | 不满足的后果 |
|------|------|-------------|
| **LiDAR ≥ 16 线, ≥ 10Hz** | 点云密度和帧率影响匹配精度 | 点数太少匹配失败 |
| **IMU ≥ 200Hz** | 高频 IMU 保证帧间预测精度 | 扫描间漂移过大 |
| **LiDAR 和 IMU 刚性固定** | 两者不能有相对运动 | 外参变化，轨迹错乱 |
| **知道或标定 LiDAR→IMU 外参** | 6DOF 平移+旋转 | 转动力臂引入 10cm+ 误差 |
| **知道 IMU 内参 (噪声/偏置)** | 滤波器参数需匹配真实 IMU | 状态估计精度大幅下降 |
| **RTK 能稳定 fix, ≥ 5Hz** | Z 轴绝对约束的可靠性 | RTK 中断期间 Z 继续漂 |
| **工控机/笔记本 CPU ≥ 4 核** | FAST-LIO2 实时需要 | 掉帧，建图失败 |
| **硬盘 ≥ 100GB 空闲** | rosbag + PCD 输出 | 录到一半没空间 |
...
1. **确认传感器型号和参数**
   - LiDAR：几线？频率？IP 地址？ROS 驱动？
   - IMU：型号？频率？内置还是外置？topic 名？
   - RTK：基站还是 NTRIP？频率？输出 topic？
   - **外参**：LiDAR 到 IMU 的平移向量和旋转矩阵（问厂商/查 URDF）

2. **准备配置文件**
   - 根据实际 LiDAR/IMU 型号改 `config/robosense16.yaml`：
     - `lid_topic`, `imu_topic`
     - `scan_line`, `scan_rate`
     - `extrinsic_T`, `extrinsic_R`（必须填真实值）
     - `acc_cov`, `gyr_cov`（有 IMU 内参则填入真实值）
...
4. **设备安装**
   - LiDAR 和 IMU 刚性固定在机器狗上，螺丝锁死
   - 确认连接线不松动
   - RTK 天线架好（如果是基站）/ NTRIP 账号登录
   - 笔记本连上传感器网络

5. **通电检查数据流**
   ```bash
   source /opt/ros/noetic/setup.bash
   rostopic hz <lidar_topic>    # 必须稳定 10Hz
   rostopic hz <imu_topic>      # 必须 ≥200Hz
   rostopic hz <rtk_topic>      # 5~20Hz
   ```
   频率不稳定 → 不要开始采集，排查线缆/驱动

6. **RTK 收敛**
   - 室外等待 RTK fix（30 秒~2 分钟）
   - 确认 `covariance[0] < 0.1`（定位精度 <10cm）
   - RTK 没 fix 之前不要开始
...
8. **移动采集要点**
   - **先开 rosbag，确认在录，再让狗动**
   - 启动后静止 5~10 秒（IMU 初始化 + 偏置收敛）
   - 匀速 0.3~0.5 m/s（快了丢帧，快了飘）
   - 转弯角速度 < 30°/s
   - **必须走回起点（回环）** ← 这是最关键的一步
   - 回到起点后静止 5 秒再结束
...
10. **回放 bag 生成 PCD**
    ```bash
    roslaunch fast_lio mapping_robosense16.launch &
    rosbag play <bag_file> -r 1 --clock
    # FAST-LIO2 退出后 PCD 在 FAST_LIO/PCD/scans.pcd
    ```

11. **回环修正（两遍法）**
    - 第一遍播完，node 不重启
    - 立即播第二遍（Rviz reset 后重新 `rosbag play`）
    - 第二遍的 `/corrected_path` 就是闭合轨迹
    - Rviz 中查看绿色轨迹是否闭合
...
12. **三维建模**
    ...
    python3 ~/mine_project/scripts/offline_reconstruct.py \
        ...
        --voxel 0.2 --depth 10 \
        --output ~/outputs/tunnel_model.ply
    ```
...
```
        ...
        │  (ScanContext+GTSAM)│
        ├──────────────────┤
        │    RTK Z 约束     │  ← 在线拉住, 厘米级绝对高程
        │  (有 RTK 时启用)   │
        ├──────────────────┤
        │   正确外参+IMU参数  │  ← 减轻漂移, 滤波器和硬件匹配
        │ (标定文件写入config) │
        ├──────────────────┤
        │   走回环路线       │  ← 操作层面, 给回环检测机会
        │  (必须走回起点)    │
        ├──────────────────┤
        │  LiDAR+IMU 刚性连接 │  ← 物理基础, 不可妥协
        └──────────────────┘
```
